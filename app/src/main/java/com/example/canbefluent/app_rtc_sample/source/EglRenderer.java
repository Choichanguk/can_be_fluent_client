package com.example.canbefluent.app_rtc_sample.source;
...
public class EglRenderer implements Callbacks {
                ...
                ThreadUtils.invokeAtFrontUninterruptibly(this.renderThreadHandler, new Runnable() {
                    public void run() {
                        if (sharedContext == null) {
                            ...
                            EglRenderer.this.eglBase = new EglBase10((org.webrtc.EglBase10.Context)null, configAttributes);
                        } else {
    ...
    public void addFrameListener(final EglRenderer.FrameListener listener, final float scale) {
        this.postToRenderThread(new Runnable() {
            public void run() {
                EglRenderer.this.frameListeners.add(new EglRenderer.FrameListenerAndParams(listener, scale, EglRenderer.this.drawer));
            }
        });
    }
    ...
    public void addFrameListener(final EglRenderer.FrameListener listener, final float scale, final GlDrawer drawer) {
        this.postToRenderThread(new Runnable() {
            public void run() {
                EglRenderer.this.frameListeners.add(new EglRenderer.FrameListenerAndParams(listener, scale, drawer));
            }
        });
    }
    ...
    private void notifyCallbacks(I420Frame frame, float[] texMatrix) {
        if (!this.frameListeners.isEmpty()) {
            ArrayList<EglRenderer.FrameListenerAndParams> tmpList = new ArrayList(this.frameListeners);
