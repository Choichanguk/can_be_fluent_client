...
public class followerAdapter extends RecyclerView.Adapter<followerAdapter.ViewHolder> {
    ...
    // 리스너 객체 참조를 저장하는 변수
    private followerAdapter.OnItemClickListener mListener = null ;


    // 생성자에서 데이터 리스트 객체를 전달받음.
    public followerAdapter(ArrayList<follow_item> list, Context context) {
        mData = list ;
        mContext = context;
    }
    ...
    public class ViewHolder extends RecyclerView.ViewHolder {
        ...
        ViewHolder(final View itemView) {
            ...
            btn_unfollowing = itemView.findViewById(R.id.btn_unfollow);
            ...
            btn_unfollowing.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    int pos = getAdapterPosition() ;
                    if (pos != RecyclerView.NO_POSITION) {
    ...
    public followerAdapter.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        View view = inflater.inflate(R.layout.follower_item, parent, false) ;	// 리사이클러뷰에 들어갈 아이템뷰의 레이아웃을 inflate.
    ...
    public void onBindViewHolder(followerAdapter.ViewHolder holder, int position) {
        ...
        if(item.getIs_follow().equals("0")){
            holder.btn_unfollowing.setVisibility(View.GONE);
            holder.btn_follow.setVisibility(View.VISIBLE);
        }
        // 맞팔이라면
        else{
            holder.btn_unfollowing.setVisibility(View.VISIBLE);
